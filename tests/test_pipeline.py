...
try:
    ...
    print(f"   ✓ After preprocessing: {df.shape[0]} rows × {df.shape[1]} cols")
    assert df.shape[0] == 1553, f"Expected 1553 rows, got {df.shape[0]}"
    print(f"   ✓ Row count matches: 1553")
    ...
    # Check if we have the core columns (some encoding columns will be added)
    core_cols_present = [c for c in expected_cols if c in df.columns]
    print(f"   ✓ Core columns present: {len(core_cols_present)}/{len(expected_cols)}")
    ...
    print("Expected: 1553 rows × 24 cols (approx with MLB features)")
